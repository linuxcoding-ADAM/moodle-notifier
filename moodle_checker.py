# FINAL, FULLY ROBUST RAILWAY SCRIPT (All fixes included)
...
def run_check():
    ...
    with requests.Session() as s:
        try:
            ...
            return # Exit this check if scraping fails
    ...
    if new_announcements:
        ...
        for item in reversed(new_announcements):
            ...
            # ONLY if the send was successful, add the ID to memory and save immediately
            if success:
                seen_ids.add(item['id'])
                save_seen_ids(seen_ids)
                logging.info(f"Successfully processed and saved ID: {item['id']}")
            else:
                logging.warning(f"Failed to send notification for {item['id']}. Will retry on next check.")
